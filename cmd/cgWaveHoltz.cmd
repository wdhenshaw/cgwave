...
GetOptions( "omega=f"=>\$omega,"x0=f"=>\$x0,"y0=f"=>\$y0,"z0=f"=>\$z0,"beta=f"=>\$beta,"numPeriods=i"=>\$numPeriods,            "omegaSOR=f"=>\$omegaSOR,"tol=f"=>\$tol,"ad4=f"=>\$ad4,"cfl=f"=>\$cfl,"tp=f"=>\$tp,"iMode=i"=>\$imode,            "solver=s"=>\$solver,"kx=f"=>\$kx,"ky=f"=>\$ky,"kz=f"=>\$kz,"maxIt=i"=>\$maxIt,"matlab=s"=>\$matlab,            "go=s"=>\$go,"forcing=s"=>\$forcing,"bc=s"=>\$bc,"ts=s"=>\$ts,"orderInTime=i"=>\$orderInTime,            "dtMax=f"=>\$dtMax,"adjustOmega=i"=>\$adjustOmega ); 
# 
...
if( $forcing eq "gaussian"){  $cmd = "user defined forcing...\n"       . " gaussian sources\n"       . "   1\n"       . " $amp $beta $omega $p $x0 $y0 $z0 $t0\n"       . " exit"; } 
# 
...
gaussian sources 
1 
273.94756 400 5.234 1 .75 .75 0.5 0 
exit 
# 
exit 
# --- end cgWave setup  --- 
