...
if 0;
#!/usr/bin/perl
# perl program to run regression tests for WAVEHOLTZ
# 
...
$testName="all";
$testsFile="testsWaveHoltz"; # location of tests
$replace="";
$verbose=1; 
$solver=""; 
...
printf("\n");
printf("================================================================================\n");
printf("This perl script will run regression tests for WAVEHOLTZ \n");
printf("      (The lists of tests is in the file named 'tests') \n");
printf("  Usage: \n");
printf("    checkWaveHoltz.p -test=testName -replace -check=<checkFileDirectory> -solver=name -verbose=<num> -testsFile=<s>\n");
...
#    $setEnvironment = "set path = \(\".\" \"/usr/apps/pgi/3.3/linux86/bin\" \$path\); echo \"path=\$path\"; set args = \(\"$moduleArgs\" \"$OvertureDistribution\"\); source /home/henshaw/bin/module;";
    ...
    if( $returnValue == 0 )
    {
      if( (-e $checkFileName ) && ($replace eq "")  )
      {
        ...
        if( $verbose ){ printf("COMPARE files: \"$checkCheckFiles $checkFile $checkFileName -tol=$tol $pipeOutput\"\n"); }
        ...
        if( $verbose ){ printf("Header new:[$lineNew]\n"); }
        if( $verbose ){ printf("Header old:[$lineOld]\n"); }
