#
# RPI GRID -- THIS VERSION WITH SHARP CORNERS
#
#   Hollow objects that have the form
#               "R" "P" "I" 
#
#
# usage: ogen [noplot] rpiGridSharp -factor=<num> -order=[2/4/6/8] -interp=[e/i]  -periodic=[|p|np|pn]
# 
# examples:
#     ogen -noplot rpiGridSharp -interp=e -order=2 -factor=4 
#     ogen -noplot rpiGridSharp -interp=e -order=2 -factor=6 
#     ogen -noplot rpiGridSharp -interp=e -order=2 -factor=8
#
#     ogen -noplot rpiGridSharp -interp=e -order=2 -xa=.5 -xb=4 -factor=4 
# 
$prefix="rpiGridSharp"; 
$order=2; $factor=1; $interp="i"; # default values
$orderOfAccuracy = "second order"; $ng=2; $interpType = "implicit for all grids";
$name=""; $xa=0; $xb=8.5; $ya=-2; $yb=2.; $ml=0;
$periodic=""; 
$numGhost=-1; # if this value is set, then use this number of ghost points
# 
# get command line arguments
GetOptions( "order=i"=>\$order,"factor=f"=> \$factor,"xa=f"=> \$xa,"xb=f"=> \$xb,"ya=f"=> \$ya,"yb=f"=> \$yb,\
            "interp=s"=> \$interp,"name=s"=> \$name,"periodic=s"=>\$periodic,"numGhost=i"=>\$numGhost );
# 
if( $order eq 4 ){ $orderOfAccuracy="fourth order"; $ng=3; }\
elsif( $order eq 6 ){ $orderOfAccuracy="sixth order"; $ng=4; }\
elsif( $order eq 8 ){ $orderOfAccuracy="eighth order"; $ng=6; }
if( $interp eq "e" ){ $interpType = "explicit for all grids"; }
# 
$suffix=""; 
if( $periodic eq "p" ){ $suffix = "p"; }
if( $periodic eq "np" ){ $suffix = "np"; }
if( $periodic eq "pn" ){ $suffix = "pn"; }
$suffix .= ".order$order";
if( $numGhost ne -1 ){ $ng = $numGhost; } # overide number of ghost
if( $numGhost ne -1 ){ $suffix .= ".ng$numGhost"; } 
if( $name eq "" ){$name = $prefix . "$interp$factor" . $suffix . ".hdf";}
# 
$ds=.1/$factor;
#
# -- convert a number so that it is a power of 2 plus 1 --
#    ml = number of multigrid levels 
$ml2 = 2**$ml; 
sub intmg{ local($n)=@_; $n = int(int($n+$ml2-2)/$ml2)*$ml2+1; return $n; }
sub max{ local($n,$m)=@_; if( $n>$m ){ return $n; }else{ return $m; } }
sub min{ local($n,$m)=@_; if( $n<$m ){ return $n; }else{ return $m; } }
#
create mappings
#
#  Background 
#
  rectangle
    mappingName
      backGround
   set corners
    $xa $xb $ya $yb
   lines
    $nx = int( ($xb-$xa)/$ds +1.5 ); 
    $ny = int( ($yb-$ya)/$ds +1.5 ); 
    $nx $ny
    boundary conditions
      if( $periodic eq "p" ){ $bc ="-1 -1 -1 -1"; }\
      elsif( $periodic eq "np" ){ $bc ="1 2 -1 -1"; }\
      elsif( $periodic eq "pn" ){ $bc ="-1 -1 3 4"; }else{ $bc="1 2 3 4"; }   
      $bc     
  exit
#
$fluidGridNames="#";
$solidGridNames="#"; 
$solidDomains="#";
# $shareInterface=99;
# $bcInterface=99;
$startBC=5; # start bC's for bodies at this value.
$shareInterface=$startBC-1;
$bcInterface=$startBC-1;
#
# I-beam, beam (upper), beam (lower), split-ring, cross
#
# --- build R-beam grid 
#
$count=0; 
$degree=3; # degree of nurbs
#
#   ---- R-Beam ----
#
$edgeWidth=0.5; $middleWidth=0.25; 
$slantLength=0.5; $bottomHeight=0.5; 
$topHeight=0.25;
$cx=2; $cy=$edgeWidth;
$coreRadius=.25; $xCore=.5*$middleWidth; $yCore=0.; # offset core from center
# --- define curve ---
# include $ENV{CG}/mp/runs/diskInAChannel/solidRBeamCurve.h
include $ENV{CGWAVE}/runs/scattering/solidRBeamCurveSharp.h
#  --- build grids ---
include $ENV{CGWAVE}/runs/scattering/solidBodyGridsSharp.h
#
#   ---- P-Beam ----
#
$edgeWidth=0.5; $middleWidth=0.25; 
$slantLength=0.5; $bottomHeight=0.5; 
$topHeight=0.25;
$cx=4.25; $cy=$edgeWidth;
$coreRadius=.25; $xCore=.5*$middleWidth; $yCore=0.; # offset core from center
# --- define curve ---
include $ENV{CG}/mp/runs/diskInAChannel/solidPBeamCurve.h
#  --- build grids ---
include $ENV{CG}/mp/runs/diskInAChannel/solidBodyGrids.h
#
#   ---- I-Beam ----
#
$centerWidth=$edgeWidth; $edgeHeight=$edgeWidth;
$centerHeight=$height-2*$edgeHeight; 
$edgeWidth=$width; 
$angle=0; 
$cx=6.5; $cy=0;
$coreRadius=.15; $xCore=0; $yCore=.5*$centerHeight+.25*$edgeHeight; # offset core from center
# --- define curve ---
include $ENV{CG}/mp/runs/diskInAChannel/solidIBeamCurve.h
#  --- build grids ---
include $ENV{CG}/mp/runs/diskInAChannel/solidBodyGrids.h
# 
exit 
#
generate an overlapping grid
  backGround
  # R : 
  fluidInterface1
  # $fluidGridNames
  done
  change parameters
    ###$solidDomains
    # --- Make just one solid domain ---
    # specify a domain
    #   solidDomain
    #     solidBackGround1
    #     # solidCore1
    #     solidInterface1
    #     solidBackGround2
    #     # solidCore2
    #     solidInterface2
    #     solidBackGround3
    #     # solidCore3
    #     solidInterface3
    # done
    # #
    # specify a domain
    #  fluidDomain
    #    backGround
    #    $fluidGridNames
    # done
    # choose implicit or explicit interpolation
    interpolation type
      $interpType
    order of accuracy 
      $orderOfAccuracy
    ghost points
      all
      $ng $ng $ng $ng $ng $ng 
    exit
  # pause
  open graphics

  compute overlap
exit
#
save an overlapping grid
$name
rpiGridSharp
exit
