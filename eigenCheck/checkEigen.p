...
if 0;
#!/usr/bin/perl
# perl program to run regression tests for EIGENWAVE
# 


# $fileName          = $ARGV[0];
$checkFileDirectory = ".";
$resultsDir = "results";    # put results here 
$testName="all";
$testsFile="testsEigenWave"; # location of tests
$replace="";
$verbose=1; 
$solver=""; 
...
printf("\n");
printf("================================================================================\n");
printf("This perl script will run regression tests for EIGENWAVE \n");
printf("      (The lists of tests is in the file named 'tests') \n");
printf("  Usage: \n");
printf("    checkEigen.p -test=testName -replace -check=<checkFileDirectory> -solver=name -verbose=<num> -testsFile=<s>\n");
...
foreach $cmdCommand ( @cmdFiles )
{
  ...
  $checkFileName = "$resultsDir/$checkFileName";
  $cmdName = $cmdCommand;
  if( $cmdName =~ /\.cmd/ )
  {
...
#    $setEnvironment = "set path = \(\".\" \"/usr/apps/pgi/3.3/linux86/bin\" \$path\); echo \"path=\$path\"; set args = \(\"$moduleArgs\" \"$OvertureDistribution\"\); source /home/henshaw/bin/module;";
    ...
    if( $parallel eq "" )
    {
      if( $verbose ){ printf("running:  $program -noplot $cmd >! $resultsDir/$runTimeOutput.out\n"); }
      $returnValue = system("csh -f -c '$program -noplot $cmd >! $resultsDir/$runTimeOutput.out'");
    }
    else
    {
      ...
      if( $verbose ){ printf("running: $mpirunCommandNP $program -noplot $cmd >&! $resultsDir/$runTimeOutput.out\n"); }
      $returnValue = system("csh -f -c '$mpirunCommandNP $program -noplot $cmd>&! $resultsDir/$runTimeOutput.out'");
    }
    ...
    if( $returnValue == 0 )
    {
      if( (-e $checkFileName ) && ($replace eq "")  )
      {
        ...
        printf("COMPARE files: \"$checkCheckFiles $checkFile $checkFileName -tol=$tol $pipeOutput\"\n");

        # $diff = system "$checkCheckFiles $checkFile $checkFileName -tol=$tol $pipeOutput";

        $diff=0; 
        open(fileNew,"$checkFile") || die print "unable to open $checkFile\n";
        open(fileOld,"$checkFileName") || die print "unable to open $checkFileName\n";

        my $fileNew = $checkFile;
        open $infoNew, $fileNew or die "Could not open $file: $!";
        my $fileOld = $checkFileName;
        open $infoOld, $fileOld or die "Could not open $file: $!";

        $lineNew = <$infoNew>; chop($lineNew); 
        $lineOld = <$infoOld>; chop($lineOld);

        printf("Header new:[$lineNew]\n");
        printf("Header old:[$lineOld]\n");

        # ------ Here is a first pass -----
        while( $lineNew = <$infoNew> )
        {
          chop($lineNew); 
          $lineOld = <$infoOld>; chop($lineOld);
          if( $lineNew != $lineOld )     
  ...
  {
    printf("checkEigen.p: ERROR: The command file $cmdName.cmd not found! (maybe a blank line?)\n");
  }
}
...
if ( $numberOfErrors == 0 )
{
  ...
  @FILENAMES = ("eigenWave.check", "cgWave.check", "cgWave.debug", "cgWave.out", 
                "cgWaveHoltz.check", "cgWaveHoltz.debug", "cgWaveHoltz.out", "cgWaveHoltz.cmd", 
