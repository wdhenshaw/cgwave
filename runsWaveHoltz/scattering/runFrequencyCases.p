...
$maxIterations=1000; 
$go = "fk";
$x0==.55; $y0=0.6; # location of Gaussian
$suffix="";        # suffix for Matlab file name 
...
#   $gridName[$i] = "square512";   $freqArray[$i]=120;  $ds[$i]=1./512;  $np[$i]= 2; $i=$i+1;
#   $gridName[$i] = "square512";   $freqArray[$i]=160;  $ds[$i]=1./512;  $np[$i]= 2; $i=$i+1;
#   $gridName[$i] = "square1024";  $freqArray[$i]=250;  $ds[$i]=1./1024; $np[$i]= 4; $i=$i+1;
#   $gridName[$i] = "square1024";  $freqArray[$i]=300;  $ds[$i]=1./1024; $np[$i]= 4; $i=$i+1;
#   $gridName[$i] = "square2048";  $freqArray[$i]=400;  $ds[$i]=1./2048; $np[$i]= 8; $i=$i+1;
#   $gridName[$i] = "square2048";  $freqArray[$i]=500;  $ds[$i]=1./2048; $np[$i]= 8; $i=$i+1;
...
# GENERAL ORDER  -- compute the max frequency for each grid based on the PPW 
@Nv = (128,256,512,1024,2048,4096,8192);
@np = (  1,  2,  4,   8,  16,  32,  32); # number of processors
for( $g=0; $g<=$#Nv; $g++ )
{
 # compute the max freqency supported on this grid according to the PPW rule of thumb
 $N = $Nv[$g];
 $dx = $domainSize/$N;
 $mu = $order/2; 
 $bp = 2./( ($mu+1)*($mu+1) * nChooseK(2*$mu+2,$mu+1) );
 $freq = ($c/$domainSize) *  ( ($domainSize/$dx)*(2*$epsr/$bp)**(1/$order) )**($order/($order+1));

 # truncate to an integer
 $freq = int($freq);

 printf("i=$i: N=$N dx=$dx freq=$freq\n");

 $gridName[$i] = "square$N";   $freqArray[$i]=$freq;   $ds[$i]=$dx;  $i=$i+1;
}
...
for( $i=$startCase; $i<=$endCase; $i=$i+1 )  
{
  ...
  $matlab = "$grid" ."O$order" . "B$bc" . "Freq$freq" . $suffix;
  ...
  $mu = $order/2; 
  $bp = 2./( ($mu+1)*($mu+1) * nChooseK(2*$mu+2,$mu+1) );
  $ppwRuleOfThumb[$i] = 2.*$pi*( $pi*$bp*$NLambda[$i]/$epsr)**(1./$order);
...
for( $i=$startCase; $i<=$endCase; $i=$i+1 )  
{
  printf("case %2d : order=$order omega=%10.3e domainSize=%10.2e Nlambda=%10.2e, ppw=lambda/dx=%5.1f ppw(rule-of-thumb)=%5.1f (epsr=%g) grid=$gridName[$i]\n",
           $i,$freqArray[$i],$domainSize,$NLambda[$i],$ppw[$i],$ppwRuleOfThumb[$i],$epsr);  
}

# make a Latex Table
printf("\n");
printf("%% ----------- Table generated by runFrequencyCases ---------------\n");
printf("\\begin{tabular}{|c|c|c|c|c|c|}\n");
printf("\\hline\n");
printf("\\multicolumn{6}{|c|}{\$\\eps=$epsr\$, \$L=$domainSize\$, order=$order} \\\\ \\hline\n");
printf("   grid      &   \$\\dx\$   & \$N_\\Lambda\$& \$\\omega\$ &  \\PPW   &   \$\\PPW_$order\$   \\\\ \\hline\n" );
for( $i=$startCase; $i<=$endCase; $i=$i+1 )  
{
  printf(" %10s  & %8.2e  &    \%5.1f   &   \%6.1f  &  \%5.1f  &  \%5.1f  \\\\ \n", $gridName[$i], $dxv[$i], $NLambda[$i], $freqArray[$i], $ppw[$i],$ppwRuleOfThumb[$i]);
}
printf("\\hline\n");
