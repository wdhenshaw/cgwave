...
int CgWave::getHelmholtzForcing( realCompositeGridFunction & f  )
{
    ...
    {
        printF("CgWave::getHelmholtzForcing: ***EVALUATE THE HELMHOLTZ FORCING***\n");
  ...
  // -------- Fill in boundary conditions for the direct Helmholtz solver ---------
    ...
    int assignBCForImplicit = 1; 
    if( dbase.get<int>("solveHelmholtzDirect") )
        assignBCForImplicit = 2;  // added May 11, 2026
      ...
      // int gridIsImplicit = 0; 
        ...
        //   D+t (Dx ) w + A+( ... )
          ...
          // printF("\n XXXXXXX cEM2=%e XXXXXXX\n\n",cEM2);
            ...
            real *pun = fLocal.getDataPointer();
