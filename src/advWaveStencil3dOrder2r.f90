! This file automatically generated from advWaveStencil.bf90 with bpp.
  subroutine advWaveStencil3dOrder2r( nd,n1a,n1b,n2a,n2b,n3a,n3b,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,mask,xy,rsxy,um,u,un,f,sc,v,vh,lapCoeff,etax,etay,etaz,bc,frequencyArray,pdb,ipar,rpar,ierr )
 !======================================================================
...
! Define variables to valuate stencil coefficients, dim=3, order=2, gridType=Rectangular
   ...
   if( gridIsImplicit.eq.0 )then 
     ! ------- EXPLICIT update the solution ---------
       if( orderInTime.eq.2 .and. orderOfAccuracy.gt.2 )then
         ...
         !   updateWaveOpt(3,2,2,rectangular,NOFORCING)
         ! else 
         !   updateWaveOpt(3,2,2,rectangular,FORCING)
         ! end if
       else
         if( addForcing.eq.0 )then
             if( ( .true. .or. debug.gt.3) .and. t.lt.2*dt )then
               write(*,'("advWaveStencil: ADVANCE dim=3 order=2 orderInTime=2, grid=rectangular... t=",e10.2)') t
             end if
...
un(i1,i2,i3,m)=  - um(i1,i2,i3,m)+ scr(  5)*u(i1+0,i2+0,i3-1,m)                                + scr( 11)*u(i1+0,i2-1,i3+0,m)                                + scr( 13)*u(i1-1,i2+0,i3+0,m) + scr( 14)*u(i1+0,i2+0,i3+0,m) + scr( 15)*u(i1+1,i2+0,i3+0,m)+ scr( 17)*u(i1+0,i2+1,i3+0,m)                                + scr( 23)*u(i1+0,i2+0,i3+1,m)                                
               end do
               end do
               end do
             ! endLoopsMask()
...
! Stencil: nd=3, orderOfAccuracy=2, gridType=Rectangular
un(i1,i2,i3,m)=  - um(i1,i2,i3,m)+ scr(  5)*u(i1+0,i2+0,i3-1,m)                                + scr( 11)*u(i1+0,i2-1,i3+0,m)                                + scr( 13)*u(i1-1,i2+0,i3+0,m) + scr( 14)*u(i1+0,i2+0,i3+0,m) + scr( 15)*u(i1+1,i2+0,i3+0,m)+ scr( 17)*u(i1+0,i2+1,i3+0,m)                                + scr( 23)*u(i1+0,i2+0,i3+1,m)                                +dtSq*fv(m)
               end do
               end do
               end do
             ! endLoopsMask()
