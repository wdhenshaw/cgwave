! This file automatically generated from advWave.bf90 with bpp.
    subroutine advWave2dOrder8c( nd,n1a,n1b,n2a,n2b,n3a,n3b,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,mask,xy,rsxy,um,u,un,f,stencilCoeff,v,vh,lapCoeff,etax,etay,etaz,bc,frequencyArray,pdb,ipar,rpar,ierr )
   !======================================================================
       ...
       if( gridIsImplicit.eq.0 )then 
            ...
            ! FD26 : second-order in time and sixth-order in space
             if( addForcing.eq.0 )then
                if( (orderOfAccuracy.eq.6 .or. debug.gt.3) .and. t.lt.2*dt )then
                  write(*,'("advWave: ADVANCE dim=2 order=8 orderInTime=2, grid=curvilinear... t=",e10.2)') t
                end if
                   ...
                   ! ---- EIGTH 8 ---
                     ! --- EIGTH-8 TWO DIMENSIONS ---
                       ! orderInSpace==8 and orderInTime==2                                                   
                         un(i1,i2,i3,m)= cn*u(i1,i2,i3,m) +cm*um(i1,i2,i3,m) + cLap*( uxx82(i1,i2,i3,m)  +  uyy82(i1,i2,i3,m) ) 
               ...
               ! #End
                   ...
                   ! ---- EIGTH 8 ---
                     ! --- EIGTH-8 TWO DIMENSIONS ---
                       ! orderInSpace==8 and orderInTime==2                                                   
                         un(i1,i2,i3,m)= cn*u(i1,i2,i3,m) +cm*um(i1,i2,i3,m) + cLap*( uxx82(i1,i2,i3,m)  +  uyy82(i1,i2,i3,m) ) +cf*fv(m)
                 ! write(*,'("i1,i2=",2i3," u-ue=",e10.2)') i1,i2,u(i1,i2,i3,m)-ev(m)
                 ! write(*,'(" uxx-uxxe =",e10.2)') uxx22r(i1,i2,i3,0)-evxx(m)
                 ! OGDERIV2D( 0,0,0,0,i1,i2,i3,t+dt, ec, ev(m)  )
                 ! write(*,'(" un-ue=",e10.2)') un(i1,i2,i3,m)-ev(m)
             ...
             if( addForcing.eq.0 )then
                 if( (orderOfAccuracy.eq.6 .or. debug.gt.3) .and. t.lt.2*dt )then
                   write(*,'("advWave: ADVANCE dim=2 order=8 orderInTime=8, grid=curvilinear... t=",e10.2)') t
                 end if
                 ! --- TAYLOR TIME-STEPPING --- 
                 m=0 ! component number 
                 ec = 0 ! component number
         ...
         if( takeImplicitFirstStep.eq.1 )then
           if( orderInTime.eq.2 )then
             ...
             if( (debug.gt.3) .and. t.lt.2*dt )then
               write(*,'("advWave: ADVANCE IMPLICIT dim=2 order=8 orderInTime=8, grid=curvilinear, t=",e10.2)') t
             end if
         ...
         if( useImplicitUpwindDissipation.eq.1 )then
             if( (.true. .or. debug.gt.3) .and. t.lt.2*dt )then
               write(*,'("addUpwindDissImplicit: UPWIND DISS dim=2 order=8 grid=curvilinear... t=",e10.2)') t
       ...
       if( preComputeUpwindUt.eq.1 )then
         ! precompute Ut in upwind dissipation,  (uses v=uDot computed above)
           if( debug.gt.3 .and. t.lt.4*dt )then
             write(*,'("addUpwindDiss: UPWIND DISS using u.t=v dim=2 order=8 grid=curvilinear... t=",e10.2)') t
             write(*,'(" adxSosup=",3e12.4)') adxSosup(0), adxSosup(1),adxSosup(2)
           end if
           m=0 ! component number 
           ec = 0 ! component number
         ...
         ! compute Ut inline in Gauss-Seidel fashion (this is more stable)
           if( debug.gt.3 .and. t.lt.4*dt )then
             write(*,'("addUpwindDiss: UPWIND DISS using u.t=Dztu dim=2 order=8 grid=curvilinear... t=",e10.2)') t
             write(*,'(" adxSosup=",3e12.4)') adxSosup(0), adxSosup(1),adxSosup(2)
           end if
           m=0 ! component number 
           ec = 0 ! component number
