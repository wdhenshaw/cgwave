...
userDefinedForcing( realArray & f, int iparam[], real rparam[] )
{
  ...
  // const Real fSign = 1.;
    ...
    if( true )
    {
        printF("\n ##### userDefinedForcing: t=%9.3e option=[%s] fSign=%g ####\n\n",t,(const char*)option,fSign);
    }
  ...
  // restrict bounds to local processor, include parallel ghost pts:
    ...
    // Add the Gaussian source terms to fLocal
        ...
        // const Real omega= gaussianParameters(2,m,freq); 
                ...
                const Real betaSq = beta*beta; // June 19, 2023 : switch beta to beta^2 to match matlab 
        ...
        // Use frequencyArray so we get any adjusted frequencies: --> doesn't matter if only evaluated at t=0
                ...
                if( true || t0 < 2.*dt )
                    printF("\n >>> Gaussian source %i, freq=%d: setting a=%8.2e, beta=%8.2e, omega=%8.2e, p=%8.2e, x0=%8.2e, y0=%8.2e, "
                                  "z0=%8.2e, t0=%8.2e, t=%8.2e, fSign=%g\n\n", m,freq,a,beta,omega,p,x0,y0,z0,t0,t,fSign);
        ...
        // const Real sint=sin(omega*(t-t0));
          ...
          // --- 2D ---
            ...
            // real g = a*cost*exp( -beta*pow( rSq, p ) );
                        real aExp = a*exp( -betaSq*pow( rSq, p ) );
            // real g = aExp*cost;
                        real g = aExp*cost;
        ...
        if( false )
        {
            ::display(fLocal,"Gaussian forcing","%5.2f ");
        }
    }
      ...
      // We save parameters in the data base 
            ...
            else
            {
                printF("The Gaussian source in 3D is of the form:\n"
                              " g(x,y,z,t) = a*cos( omega*(t-t0) )*exp( -beta^2*[ (x-x0)^2 + (y-y0)^2 + (z-z0)^2 ]^p )\n"
                    );
            }
