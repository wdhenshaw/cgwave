! This file automatically generated from advWave.bf90 with bpp.
    subroutine advWave3dOrder6r( nd,n1a,n1b,n2a,n2b,n3a,n3b,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,mask,xy,rsxy,um,u,un,f,stencilCoeff,v,vh,lapCoeff,etax,etay,etaz,bc,frequencyArray,pdb,ipar,rpar,ierr )
   !======================================================================
       ...
       if( gridIsImplicit.eq.0 )then 
            ...
            ! FD26 : second-order in time and sixth-order in space
             if( addForcing.eq.0 )then
                if( (orderOfAccuracy.eq.6 .or. debug.gt.3) .and. t.lt.2*dt )then
                  write(*,'("advWave: ADVANCE dim=3 order=6 orderInTime=2, grid=rectangular... t=",e10.2)') t
                end if
               ...
               ! #End
                   ! ---- SIXTH 6 ---
                     ! --- SIXTH-6 THREE DIMENSIONS ---
                       ...
                       ! Turn off for now: 
                       ! getSixthDerivatives3d(6,rectangular,evalMetrics,i1,i2,i3)
                       write(*,'("advWave: order=6, orderInTime=2 FINISH ME")')
                       stop 7777
                       un(i1,i2,i3,m)= 2.*u(i1,i2,i3,m)-um(i1,i2,i3,m) + cdtSq*( uxx + uyy +uzz ) + cdtPow4By12*( uxxxx + uyyyy + uzzzz + 2.*( uxxyy +uxxzz + uyyzz ) )  + cdtPow6By360*( uxxxxxx +  uyyyyyy + uzzzzzz + 3.*(uxxxxyy + uxxyyyy + uxxxxzz + uyyyyzz + uxxzzzz + uyyzzzz ) + 6.*uxxyyzz ) 
               ...
               ! #End
                   ...
                   ! ---- SIXTH 6 ---
                     ! --- SIXTH-6 THREE DIMENSIONS ---
                       ...
                       write(*,'("advWave: order=6, orderInTime=2 FINISH ME")')
                       stop 7777
                       un(i1,i2,i3,m)= 2.*u(i1,i2,i3,m)-um(i1,i2,i3,m) + cdtSq*( uxx + uyy +uzz ) + cdtPow4By12*( uxxxx + uyyyy + uzzzz + 2.*( uxxyy +uxxzz + uyyzz ) )  + cdtPow6By360*( uxxxxxx +  uyyyyyy + uzzzzzz + 3.*(uxxxxyy + uxxyyyy + uxxxxzz + uyyyyzz + uxxzzzz + uyyzzzz ) + 6.*uxxyyzz ) +cf*fv(m)
                 ! write(*,'("i1,i2=",2i3," u-ue=",e10.2)') i1,i2,u(i1,i2,i3,m)-ev(m)
                 ! write(*,'(" uxx-uxxe =",e10.2)') uxx22r(i1,i2,i3,0)-evxx(m)
                 ! OGDERIV2D( 0,0,0,0,i1,i2,i3,t+dt, ec, ev(m)  )
                 ! write(*,'(" un-ue=",e10.2)') un(i1,i2,i3,m)-ev(m)
             ...
             if( addForcing.eq.0 )then
                 if( (orderOfAccuracy.eq.6 .or. debug.gt.3) .and. t.lt.2*dt )then
                   write(*,'("advWave: ADVANCE dim=3 order=6 orderInTime=6, grid=rectangular... t=",e10.2)') t
                 end if
                 ! --- TAYLOR TIME-STEPPING --- 
                 m=0 ! component number 
                 ec = 0 ! component number
                     ...
                     if( addForcing.eq.0 )then
                       ...
                       if( nd.eq.2 )then
                         ...
                         fv(m) = evtt(m) - csq*( evxx(m) + evyy(m)  + evzz(m) )
                          ...
                          fv(m) = fv(m) + (dtSq**2/360.)*evtttttt(m) - (cdtPow6By360/dtSq)*( evxxxxxx(m) + evyyyyyy(m) + evzzzzzz(m) + 3.*(evxxxxyy(m) + evxxyyyy(m) + evyyyyyy(m) + evyyyyzz(m) + evyyzzzz(m) )  + 6.*evxxyyzz(m)  )
                    else if( forcingOption.eq.helmholtzForcing )then
                       ! forcing for solving the Helmholtz equation   
                       ! NOTE: change sign of forcing since for Helholtz we want to solve
                       !      ( omega^2 I + c^2 Delta) w = f 
            ...
            ! FD26 : second-order in time and sixth-order in space
             if( addForcing.eq.0 )then
                if( (orderOfAccuracy.eq.2 .or. debug.gt.3) .and. t.lt.2*dt )then
                  write(*,'("advWave: ADVANCE SUPERGRID dim=3 order=6 orderInTime=2, grid=rectangular... t=",e10.2)') t
                end if
                ! --- TAYLOR TIME-STEPPING --- 
                m=0 ! component number 
                ec = 0 ! component number
               ...
               ! #End
                   ! ---- SIXTH 6 ---
                     ! --- SIXTH-6 THREE DIMENSIONS ---
                       ...
                       ! Turn off for now: 
                       ! getSixthDerivatives3d(6,rectangular,evalMetrics,i1,i2,i3)
                       ! un(i1,i2,i3,m)= 2.*u(i1,i2,i3,m)-um(i1,i2,i3,m) !                 + cdtSq*( uxx + uyy +uzz ) !                 + cdtPow4By12*( uxxxx + uyyyy + uzzzz + 2.*( uxxyy +uxxzz + uyyzz ) )  !                 + cdtPow6By360*( uxxxxxx +  uyyyyyy + uzzzzzz + 3.*(uxxxxyy + uxxyyyy + uxxxxzz + uyyyyzz + uxxzzzz + uyyzzzz ) + 6.*uxxyyzz ) !                 FV(m)
                 ! write(*,'("i1,i2=",2i3," u-ue=",e10.2)') i1,i2,u(i1,i2,i3,m)-ev(m)
                 ! write(*,'(" uxx-uxxe =",e10.2)') uxx22r(i1,i2,i3,0)-evxx(m)
                 ! OGDERIV2D( 0,0,0,0,i1,i2,i3,t+dt, ec, ev(m)  )
                 ! write(*,'(" un-ue=",e10.2)') un(i1,i2,i3,m)-ev(m)
             ...
             if( addForcing.eq.0 )then
                 if( (orderOfAccuracy.eq.2 .or. debug.gt.3) .and. t.lt.2*dt )then
                   write(*,'("advWave: ADVANCE SUPERGRID dim=3 order=6 orderInTime=6, grid=rectangular... t=",e10.2)') t
                 end if
                 ! --- TAYLOR TIME-STEPPING --- 
                 m=0 ! component number 
                 ec = 0 ! component number
         ...
         if( takeImplicitFirstStep.eq.1 )then
           if( orderInTime.eq.2 )then
             ...
             write(*,'(" advWave: TAKE IMPLICIT FIRST STEP : FINISH ME, order=6, orderInTime =",i2)') orderInTime
             stop 468
         end if           
             if( (debug.gt.3) .and. t.lt.2*dt )then
               write(*,'("advWave: ADVANCE IMPLICIT dim=3 order=6 orderInTime=6, grid=rectangular, t=",e10.2)') t
             end if
         ...
         if( useImplicitUpwindDissipation.eq.1 )then
             if( (.true. .or. debug.gt.3) .and. t.lt.2*dt )then
               write(*,'("addUpwindDissImplicit: UPWIND DISS dim=3 order=6 grid=rectangular... t=",e10.2)') t
       ...
       if( preComputeUpwindUt.eq.1 )then
         ! precompute Ut in upwind dissipation,  (uses v=uDot computed above)
           if( debug.gt.3 .and. t.lt.4*dt )then
             write(*,'("addUpwindDiss: UPWIND DISS using u.t=v dim=3 order=6 grid=rectangular... t=",e10.2)') t
             write(*,'(" adxSosup=",3e12.4)') adxSosup(0), adxSosup(1),adxSosup(2)
           end if
           m=0 ! component number 
           ec = 0 ! component number
           ...
           ! beginLoops(i1,i2,i3,n1a,n1b,n2a,n2b,n3a,n3b)
             ...
             do i1=n1a,n1b
              ...
              ! ---- SIXTH 6 ---
                ! --- THREE DIMENSIONS ---
                  un(i1,i2,i3,ec)=un(i1,i2,i3,ec)+(-70.*v(i1,i2,i3,ec)+56.*(v(i1+1,i2,i3,ec)+v(i1-1,i2,i3,ec))-28.*(v(i1+2,i2,i3,ec)+v(i1-2,i2,i3,ec))+8.*(v(i1+3,i2,i3,ec)+v(i1-3,i2,i3,ec))-(v(i1+4,i2,i3,ec)+v(i1-4,i2,i3,ec)))*adxSosup(0)+(-70.*v(i1,i2,i3,ec)+56.*(v(i1,i2+1,i3,ec)+v(i1,i2-1,i3,ec))-28.*(v(i1,i2+2,i3,ec)+v(i1,i2-2,i3,ec))+8.*(v(i1,i2+3,i3,ec)+v(i1,i2-3,i3,ec))-(v(i1,i2+4,i3,ec)+v(i1,i2-4,i3,ec)))*adxSosup(1)+(-70.*v(i1,i2,i3,ec)+56.*(v(i1,i2,i3+1,ec)+v(i1,i2,i3-1,ec))-28.*(v(i1,i2,i3+2,ec)+v(i1,i2,i3-2,ec))+8.*(v(i1,i2,i3+3,ec)+v(i1,i2,i3-3,ec))-(v(i1,i2,i3+4,ec)+v(i1,i2,i3-4,ec)))*adxSosup(2)
               end if
             end do
             end do
             end do
         ...
         ! compute Ut inline in Gauss-Seidel fashion (this is more stable)
           if( debug.gt.3 .and. t.lt.4*dt )then
             write(*,'("addUpwindDiss: UPWIND DISS using u.t=Dztu dim=3 order=6 grid=rectangular... t=",e10.2)') t
             write(*,'(" adxSosup=",3e12.4)') adxSosup(0), adxSosup(1),adxSosup(2)
           end if
           m=0 ! component number 
           ec = 0 ! component number
           ...
           ! beginLoops(i1,i2,i3,n1a,n1b,n2a,n2b,n3a,n3b)
             ...
             do i1=n1a,n1b
              ...
              ! ---- SIXTH 6 ---
                ! --- THREE DIMENSIONS ---
                  un(i1,i2,i3,ec)=un(i1,i2,i3,ec)+(-70.*Dztu(i1,i2,i3,ec)+56.*(Dztu(i1+1,i2,i3,ec)+Dztu(i1-1,i2,i3,ec))-28.*(Dztu(i1+2,i2,i3,ec)+Dztu(i1-2,i2,i3,ec))+8.*(Dztu(i1+3,i2,i3,ec)+Dztu(i1-3,i2,i3,ec))-(Dztu(i1+4,i2,i3,ec)+Dztu(i1-4,i2,i3,ec)))*adxSosup(0)+(-70.*Dztu(i1,i2,i3,ec)+56.*(Dztu(i1,i2+1,i3,ec)+Dztu(i1,i2-1,i3,ec))-28.*(Dztu(i1,i2+2,i3,ec)+Dztu(i1,i2-2,i3,ec))+8.*(Dztu(i1,i2+3,i3,ec)+Dztu(i1,i2-3,i3,ec))-(Dztu(i1,i2+4,i3,ec)+Dztu(i1,i2-4,i3,ec)))*adxSosup(1)+(-70.*Dztu(i1,i2,i3,ec)+56.*(Dztu(i1,i2,i3+1,ec)+Dztu(i1,i2,i3-1,ec))-28.*(Dztu(i1,i2,i3+2,ec)+Dztu(i1,i2,i3-2,ec))+8.*(Dztu(i1,i2,i3+3,ec)+Dztu(i1,i2,i3-3,ec))-(Dztu(i1,i2,i3+4,ec)+Dztu(i1,i2,i3-4,ec)))*adxSosup(2)
               end if
             end do
             end do
             end do
