...
int CgWaveHoltz::solveHelmholtz(int argc,char **argv)
{
    ...
    int & saveResidualToShowFile    = cgWaveHoltz.dbase.get<int>("saveResidualToShowFile");
  ...
  // null strings terminal list
    ...
    tbState[9] = saveResidualToShowFile;
  ...
  //   This is optionally used in CgWave as a known solution
    ...
    for(;;) 
    {
      ...
      // new way: 
        ...
        else if( dialog.getToggleValue(answer,"save residual to show file",saveResidualToShowFile) )
        {
            printF("Setting saveResidualToShowFile=%d ( 1 = save the residual to the show file.)\n",saveResidualToShowFile);
        }
      ...
      // getPointsPerWaveLength( frequencyArray(0), epsr, ppw, ppwRuleOfThumb, domainSize, NLambda, dx, kWaveNumber, lambdaWaveLength );
            ...
            const bool & readCheckPointFile            = cgWave.dbase.get<bool>("readCheckPointFile"); 
            const bool & saveCheckPointFile            = cgWave.dbase.get<bool>("saveCheckPointFile"); 
            const aString & readCheckPointFileName     = cgWave.dbase.get<aString>("readCheckPointFileName"); // name of the show file for reading
            const aString & saveCheckPointFileName     = cgWave.dbase.get<aString>("saveCheckPointFileName"); // name of the show file for saving
            printF(" readCheckPointFile=%d, readCheckPointFileName=[%s], writeCheckPointFile=%d, writeCheckPointFileName=[%s]\n",
                          readCheckPointFile,(const char*)readCheckPointFileName,saveCheckPointFile,(const char*)saveCheckPointFileName );
        ...
        // maxRes = cgWaveHoltz.residual( uHelmholtz );
            ...
            if( filterTimeDerivative==0 )
            {
                if( numberOfFrequencies==1 )
                {
                    printF("solveHelmholtz: max-rel-res=%9.3e (%s, numIts=%d, ave-CR=%5.2f %s, ACR=%5.2f, cpu=%8.2e(s), np=%d)\n",
          ...
          //   convergenceRate,(const char*)label,waveHoltzAsymptoticConvergenceRate);          
                    ...
                    printF("solveHelmholtz: max-rel-res=%9.3e (all freq, %s, numIts=%d, ave-CR=%5.2f %s, cpu=%8.2e(s), np=%d)\n",
                          maxRes,(const char*)schemeName,numberOfIterations,convergenceRate,(const char*)label,cpuWaveHoltz,np );
            }
            else
            {
                printF("solveHelmholtz: max-rel-residual [Re,Im]=[%8.2e,%8.2e] (%s, numIts=%d, ave-CR=%5.2f %s, cpu=%8.2e(s), np=%d)\n",maxResArray(0),maxResArray(1),
                                (const char*)schemeName,numberOfIterations,convergenceRate,(const char*)label,cpuWaveHoltz,np );
      ...
      // const int & computeEigenmodes     = cgWave.dbase.get<int>("computeEigenmodes");
            ...
            if( saveResidualToShowFile )
                numShowComponents += numberOfComponents;
      ...
      // --- Save the forcings: ---
            ...
            if( saveResidualToShowFile )
            {
                ...
                for( int freq=0; freq<numberOfComponents; freq++ )
                {        
                    q.setName(sPrintF("res%d",freq),ishow);
                    for( int grid=0; grid<cg.numberOfComponentGrids(); grid++ )
                    {
                        ...
                        bool ok=ParallelUtility::getLocalArrayBounds(q[grid],qLocal,I1,I2,I3);
                        if( ok )
                            qLocal(I1,I2,I3,ishow) = resLocal(I1,I2,I3,freq);  // residual
                    }
                    ishow++;
                }
            ...
            {
                printF("CgWaveHoltz::solveHelmholtz:ERROR: ishow=%d != numShowComponents=%d\n",ishow,numShowComponents);
            }
      ...
      // realCompositeGridFunction & q = filterTimeDerivative ? uh : cgWave.dbase.get<realCompositeGridFunction>("v");
            ...
            if( solveForScatteredField && plotTotalField )
            {
                ...
                for( int grid=0; grid<cg.numberOfComponentGrids(); grid++ )
                {
                    ...
                    OV_GET_SERIAL_ARRAY(Real,mg.vertex(),xLocal);
                    getIndex(mg.dimension(),I1,I2,I3);
                    bool ok=ParallelUtility::getLocalArrayBounds(q[grid],qLocal,I1,I2,I3,1);
