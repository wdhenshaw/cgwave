...
{
    bool & deflationInitialized = dbase.get<bool>("deflationInitialized"); //  set to true if deflation has been initialized
...
inflateSolution()
{
    ...
    const int & deflateWaveHoltz             = dbase.get<int>("deflateWaveHoltz");
    const bool & deflationInitialized        = dbase.get<bool>("deflationInitialized"); 
    const int & numToDeflate                 = dbase.get<int>("numToDeflate");
    ...
    if( !deflationInitialized )
    {
        printF("\n CgWave::inflateSolution:WARNING: deflation has not been initialized! Could be running from a check point file(?) Will initialized deflation...\n\n");
        initializeDeflation();
    // OV_ABORT("ERROR");
    }
