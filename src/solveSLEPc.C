...
solveSLEPc(int argc,char **args)
{
  ...
  // vOld.updateToMatchGrid(cg,all,all,all,numberOfFrequencies);
    ...
    const int complexProblem = filterTimeDerivative;
  ...
  //   ucg.setName(sPrintF("phi%d",i),i);
    ...
    //                 = 2  : second vector in a complex eig
      ...
      // eig(1,i) = ki;
            ...
            printF("WaveHoltz eigenvalue beta %5d : k=%19.14e %+19.14e I",i,kr,ki);
      ...
      // if (!ishermitian) { ierr = VecView(xi,viewer);CHKERRQ(ierr); }
        ...
        // FOR NOW -- FIRST SAVE in vOld so we can apply BC's .. then copy to ucg
                vOld=0.;

                const int numberOfComponents=1;
        // *new* way (from PETScSolver.bC)

        // NOTE: Seet eigSolvers/computeEigenvalues.bC to see how to treat a complex problem

                if( complexProblem )
                {
                    OV_ABORT("Finish me Bill!");
                }

                if( useMatrixUtilities )
                {
                    if( complexEigFound<=1 )
                        cgWave.vectorToGridFunction( xrv, vOld, Istart,Iend );  // use real part of eigenvector 
                    else
                        cgWave.vectorToGridFunction( xiv, vOld, Istart,Iend );  // use imaginary part of eigenvector 
                }
                else        
                {
