...
int CgWaveHoltz::outputEigenTable()
{
  ...
  // -- Compute points-per-wavelength --- 
    ...
    getPointsPerWaveLength( omega, epsr, ppw, ppwRuleOfThumb, domainSize, NLambda, dx, kWaveNumber, lambdaWaveLength );
 ...
 // const Real & dt                   = cgWave.dbase.get<real>("dtUsed");
    ...
    printF(" readCheckPointFile=%d, readCheckPointFileName=[%s], writeCheckPointFile=%d, writeCheckPointFileName=[%s]\n",
                  (int)readCheckPointFile,(const char*)readCheckPointFileName,saveCheckPointFile,(const char*)saveCheckPointFileName );

    printF(" omega=%8.2e, k=%8.2e, lambda=%8.2e, dx=%8.2e, domainSize=%8.2e, NLambda=%g, ppw=lambda/dx=%5.1f, ppw(rule-of-thumb)=%5.1f (epsr=%g)\n",
                kWaveNumber,omega,lambdaWaveLength,dx,domainSize,NLambda,ppw,ppwRuleOfThumb,epsr); 
  ...
  // uHelmholtz.updateToMatchGrid(cg,all,all,all,numberOfFrequencies); // save Helmholtz solution here
    ...
    for(;;) 
    {
      ...
      // --- DO THIS FOR NOW ** FIX ME ****************************************************
        ...
        {
      // ----- Compute points-per-wavelength for info about this grid ------ 
            const Real epsr=1e-2; // relative error tolerance
            Real & ppw            = dbase.get<Real>("ppw");            // holds actual points per wavelength
            Real & ppwRuleOfThumb = dbase.get<Real>("ppwRuleOfThumb"); // holds rule-of-thumb points per wavelength
            Real dx, domainSize, NLambda, kWaveNumber, lambdaWaveLength;
