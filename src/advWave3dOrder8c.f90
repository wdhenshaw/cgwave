! This file automatically generated from advWave.bf90 with bpp.
        subroutine advWave3dOrder8c( nd,n1a,n1b,n2a,n2b,n3a,n3b,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,mask,xy,rsxy,um,u,un,f,fa,v,vh,lapCoeff,bc,frequencyArray,ipar,rpar,ierr )
    ! subroutine advWave3dOrder8c(nd,n1a,n1b,n2a,n2b,n3a,n3b,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,!                 mask,xy,rsxy,  um,u,un, f,fa, v, vh,  bc, frequencyArray, ipar, rpar, ierr )
   !======================================================================
        ...
        ! '
         ...
         ! ------- EXPLIICT update the solution ---------
            ...
            ! FD26 : second-order in time and sixth-order in space
                            if( (orderOfAccuracy.eq.6 .or. debug.gt.3) .and. t.lt.2*dt )then
                                write(*,'("advWave: ADVANCE dim=3 order=8 orderInTime=2, grid=curvilinear... t=",e10.2)') t
                            end if
              ! --- TAYLOR TIME-STEPPING --- 
               ...
               ! write(*,'(" un-ue=",e10.2)') un(i1,i2,i3,m)-ev(m)
                              ...
                              if( (orderOfAccuracy.eq.6 .or. debug.gt.3) .and. t.lt.2*dt )then
                                  write(*,'("advWave: ADVANCE dim=3 order=8 orderInTime=8, grid=curvilinear... t=",e10.2)') t
                              end if
               ! --- TAYLOR TIME-STEPPING --- 
                              m=0 ! component number 
                              ec = 0 ! component number
         ...
         ! --- IMPLICIT: Fill in RHS to implicit time-stepping -----
                          if( (.true. .or. debug.gt.3) .and. t.lt.2*dt )then
                              write(*,'("advWave: ADVANCE IMPLICIT dim=3 order=8 orderInTime=8, grid=curvilinear... t=",e10.2)') t
                          end if
         ...
         ! --- Add contributions from upwind dissipation ---
                  if( useImplicitUpwindDissipation.eq.1 )then
                          if( (.true. .or. debug.gt.3) .and. t.lt.2*dt )then
                              write(*,'("addUpwindDissImplicit: UPWIND DISS dim=3 order=8 grid=curvilinear... t=",e10.2)') t
       ...
       !                      false=compute Ut inline in Gauss-Seidel fashion 
         ...
         ! precompute Ut in upwind dissipation,  (uses v=uDot computed above)
                      if( debug.gt.3 .and. t.lt.4*dt )then
                          write(*,'("addUpwindDiss: UPWIND DISS using u.t=v dim=3 order=8 grid=curvilinear... t=",e10.2)') t
                          write(*,'(" adxSosup=",3e12.4)') adxSosup(0), adxSosup(1),adxSosup(2)
                      end if
                      m=0 ! component number 
                      ec = 0 ! component number
         ...
         ! compute Ut inline in Gauss-Seidel fashion (this is more stable)
                      if( debug.gt.3 .and. t.lt.4*dt )then
                          write(*,'("addUpwindDiss: UPWIND DISS using u.t=Dztu dim=3 order=8 grid=curvilinear... t=",e10.2)') t
                          write(*,'(" adxSosup=",3e12.4)') adxSosup(0), adxSosup(1),adxSosup(2)
                      end if
                      m=0 ! component number 
                      ec = 0 ! component number
