! This file automatically generated from advWave.bf90 with bpp.
    subroutine advWave3dOrder6c( nd,n1a,n1b,n2a,n2b,n3a,n3b,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,mask,xy,rsxy,um,u,un,f,stencilCoeff,v,vh,lapCoeff,etax,etay,etaz,bc,frequencyArray,pdb,ipar,rpar,ierr )
   !======================================================================
       ...
       if( gridIsImplicit.eq.0 )then 
            ...
            ! FD26 : second-order in time and sixth-order in space
             if( addForcing.eq.0 )then
                if( (orderOfAccuracy.eq.6 .or. debug.gt.3) .and. t.lt.2*dt )then
                  write(*,'("advWave: ADVANCE dim=3 order=6 orderInTime=2, grid=curvilinear... t=",e10.2)') t
                end if
             ...
             if( addForcing.eq.0 )then
                 if( (orderOfAccuracy.eq.6 .or. debug.gt.3) .and. t.lt.2*dt )then
                   write(*,'("advWave: ADVANCE dim=3 order=6 orderInTime=6, grid=curvilinear... t=",e10.2)') t
                 end if
                 ! --- TAYLOR TIME-STEPPING --- 
                 m=0 ! component number 
                 ec = 0 ! component number
         ...
         if( takeImplicitFirstStep.eq.1 )then
           if( orderInTime.eq.2 )then
             ...
             if( (debug.gt.3) .and. t.lt.2*dt )then
               write(*,'("advWave: ADVANCE IMPLICIT dim=3 order=6 orderInTime=6, grid=curvilinear, t=",e10.2)') t
             end if
         ...
         if( useImplicitUpwindDissipation.eq.1 )then
             if( (.true. .or. debug.gt.3) .and. t.lt.2*dt )then
               write(*,'("addUpwindDissImplicit: UPWIND DISS dim=3 order=6 grid=curvilinear... t=",e10.2)') t
       ...
       if( preComputeUpwindUt.eq.1 )then
         ! precompute Ut in upwind dissipation,  (uses v=uDot computed above)
           if( debug.gt.3 .and. t.lt.4*dt )then
             write(*,'("addUpwindDiss: UPWIND DISS using u.t=v dim=3 order=6 grid=curvilinear... t=",e10.2)') t
             write(*,'(" adxSosup=",3e12.4)') adxSosup(0), adxSosup(1),adxSosup(2)
           end if
           m=0 ! component number 
           ec = 0 ! component number
         ...
         ! compute Ut inline in Gauss-Seidel fashion (this is more stable)
           if( debug.gt.3 .and. t.lt.4*dt )then
             write(*,'("addUpwindDiss: UPWIND DISS using u.t=Dztu dim=3 order=6 grid=curvilinear... t=",e10.2)') t
             write(*,'(" adxSosup=",3e12.4)') adxSosup(0), adxSosup(1),adxSosup(2)
           end if
           m=0 ! component number 
           ec = 0 ! component number
