...
! ===================================================================================
  ...
  !     --- local variables ----
    ...
    real c2,c4,c6,c8
    real r1,r2,r3,r4,gtt,rFactor,uLap,vLap
    real a11,a12,a21,a22
    real uett,uexx,ueyy,uezz,ueLap
    real ue1,ue2,f1,f2,det
    ...
    ! **NOTE** the bc on the right may be negative even it is not on the left (for parallel)
      ...
      do side=0,1
       ...
       ! write(*,'(" bcOpt: side,axis,bc=",3i2)') side,axis,boundaryCondition(side,axis)
        ...
        if( ( boundaryCondition(side,axis).ne.dirichlet .or. bcApproach.eq.useCompatibilityBoundaryConditions ) .and. boundaryCondition(side,axis).gt.0 )then
  ...
  !  --- Assign ghost points outside corners ---
    ...
    ! ---------------------------------
        ...
        do side2=0,2
        do side1=0,2
            if( orderOfAccuracy.eq.4. .and. boundaryCondition(side1,0).eq.neumann .and. boundaryCondition(side2,1).eq.neumann .and. ( nd.eq.2 .or. boundaryCondition(side3,2).eq.neumann ) )then
