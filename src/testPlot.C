...
//
// Test routine to plot some things
// 
...
main(int argc, char *argv[])
{
  ...
  int & numberOfFrequencies   = cgWave.dbase.get<int>("numberOfFrequencies");
  ...
  real & dt                   = cgWave.dbase.get<real>("dt");

  RealArray & sigma           = cgWave.dbase.put<realArray>("sigma");
  ...
  for( int i=0; i<n; i++ )
  {
    int j = 10 + 3*i;  // choose some points from the lambda array
    points(i,axis1)= lambda(j);
    points(i,axis2)= mu(j);
    value(i) = (i+1)/(n+1.); 
  ...
  if( true )
  {
    // psp.set(GI_POINT_COLOUR,"MEDIUMSEAGREEN");
  ...
  int mm=0; // counts components
  Real xa=-1., xb=1.; 
  for( int i=0; i<nd; i++ )
  {
    t(i)= xa + (xb-xa)*(i-1.)/(nd-1.);
    x(i,mm) = sin(twoPi*t(i));
  }
