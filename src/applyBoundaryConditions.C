                                                  ...
                                                  bool fillImplicitBoundaryConditions  /* = false */ )
{
  ...
  // }
    ...
    if( debug & 8 )
    {
        printF("\n @@ applyBoundaryConditions called at t=%9.3e, applyExplicitBoundaryConditions=%d, fillImplicitBoundaryConditions=%d,\n",t,
                  applyExplicitBoundaryConditions,fillImplicitBoundaryConditions);
        fprintf(pDebugFile,"\n @@ applyBoundaryConditions called at t=%9.3e, applyExplicitBoundaryConditions=%d, fillImplicitBoundaryConditions=%d,\n",t,
                  applyExplicitBoundaryConditions,fillImplicitBoundaryConditions);    
    }
  ...
  // bcParams.orderOfExtrapolation=orderOfAccuracy+1; *wdh* July 21, 2024
    ...
    // printF("applyBC: Call interpolate...\n");
        u.interpolate();
    // printF("applyBC: ... done call interpolate.\n");
    }
      ...
      // +++ NOTE: BCs for implicit are done in takeImplicitTimeStep +++
        ...
        // --------- EXPLICIT GRID ------------
          ...
          //   ::display(bcLocal,"applyBC: bcLocal","%3i ");
                    ...
                    if( debug & 16 )
                      ::display(uLocal,"u after bcOptWave, before abcWave",pDebugFile,"%9.2e ");
...
applyEigenFunctionBoundaryConditions( realCompositeGridFunction & u )
{
  ...
  //printF("applyEigenBC: Call interpolate...\n");
    u.interpolate();
  //printF("applyEigenBC: ... done call interpolate.\n");
