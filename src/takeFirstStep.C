...
takeFirstStep( int cur, real t )
{
    ...
    for( int grid=0; grid<cg.numberOfComponentGrids(); grid++ )
    {
      ...
      // if( forcingOption ~
            if( forcingOption==userForcing || forcingOption==helmholtzForcing )
            {
                ...
                if( debug & 4 )
                    fprintf(pDebugFile,"**** takeFirstStep: EXPLICIT FIRST STEP -- FIX ME : ADD FORCING forcingOption=%d\n",(int)forcingOption);
            }
  ...
  if( debug & 8 )
    ...
    if( debug & 4 )
    {
        ...
        fprintf(pDebugFile,"*******  CgWave::takeFirstStep; APPLY BOUNDARY CONDTIONS t=%9.3e, cur=%d, dt=%9.3e *************\n",t,cur,dt);
    }
...
takeFirstStepHelmholtz( int cur, real t )
{
    ...
    if( debug & 4 )
    {
        ...
        fprintf(pDebugFile,"*******  CgWave::takeFirstStepHelmholtz GET SOLUTION at t=dt *************\n");
    }
  ...
  // ---- WaveHoltz: initial condition is provided in v: ----
            ...
            timeSteppingMethod == explicitTimeStepping ) // May 2, 2025 added after implicit first step added for complex case
    {
        if( debug & 8 )
        {
            u[next].display(sPrintF("takeFirstStepHelmholtz: BEFORE applyBC to first step, t=%9.3e",t+dt),pDebugFile,"%9.2e ");
        }

        if( debug & 2 )
        {
            printF("~~~~~~~~~~~~~~ Apply boundary conditions after the EXPLICIT first step ~~~~~~~~~~~~~\n");
            fprintf(pDebugFile,"~~~~~~~~~~~~~~ Apply boundary conditions after the EXPLICIT first step ~~~~~~~~~~~~~\n");
        }
        bool applyExplicitBoundaryConditions=true;
        applyBoundaryConditions( u[next],u[cur], t+dt, applyExplicitBoundaryConditions );
    }
    ...
    if( debug & 8 )
    {
        u[next].display(sPrintF("takeFirstStepHelmholtz: u[next] after first step, t=%9.3e",t+dt),pDebugFile,"%9.2e ");
    }
