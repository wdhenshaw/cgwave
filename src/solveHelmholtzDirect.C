...
// =======================================================================
#define zeroMatrixCoefficients( coeff,e1,e2, i1,i2,i3 )for( int m=CE(0,e1); m<=CE(0,e2+1)-1; m++ ) coeffLocal(m,i1,i2,i3)=0.
...
// ============================================================================================
///   FILL the Neumann BC for the complex equations
// ============================================================================================
...
int CgWaveHoltz::solveHelmholtzDirect( realCompositeGridFunction & u, realCompositeGridFunction & f  )
{
    ...
    const int & orderOfExtrapolation     = cgWave.dbase.get<int>("orderOfExtrapolation");
  ...
  // --------------------------------------------------------
    cgWave.dbase.get<int>("solveHelmholtzDirect")=true;

    cgWave.getHelmholtzForcing( f );

    cgWave.dbase.get<int>("solveHelmholtzDirect")=false;
    ...
    if( 1==0 )
    {
        for( int grid=0; grid<cg.numberOfComponentGrids(); grid ++ )
        {
            ...
            ::display(fLocal,sPrintF("solveHelmholtzDirect: fLocal grid=%d",grid),pDebugFile,"%9.2e ");
        }
    }



    const aString & knownSolutionOptionCgWave = cgWave.dbase.get<aString>("knownSolutionOption"); 
    ...
    for( int grid=0; grid<cg.numberOfComponentGrids(); grid++ )
    {
        ...
        ForBoundary(side,axis)
        {
              if( mg.boundaryCondition(side,axis)==CgWave::dirichlet ||
                      mg.boundaryCondition(side,axis)==CgWave::absorbing ||  
  ...
  // -- set up extrapolation formula --
    ...
    //  ------------------------------------------------
        ...
        for( int freq=0; freq<numberOfFrequencies; freq++ )
        {
          ...
          // solver.set(OgesParameters::THEparallelSolverMethod,OgesParameters::gmres);
                    if( 1==0 )
                    {
                        solver.set(OgesParameters::THErelativeTolerance,REAL_EPSILON*10.);
                        solver.set(OgesParameters::THEabsoluteTolerance,REAL_EPSILON*10.);
                    }
      ...
      // Index Ib1,Ib2,Ib3; 
            ...
            for( int grid=0; grid<numberOfComponentGrids; grid++ )
            {
                ...
                for( int axis=0; axis<cg.numberOfDimensions(); axis++ )
                {
                    ...
                    {
                        if( bcLocal(side,axis)==CgWave::dirichlet )
                        {
                  ...
                  // const Real kz = kzBoxHelmholtz + twoPi*freq;
                      ...
                      // kz  = (rpar[3]+freq)*twoPi; 
                                ...
                                else if( knownSolutionOptionCgWave=="userDefinedKnownSolution" )
                                {
                  // new way 
                                    Real t = 0.;

                                    cgWave.getUserDefinedKnownSolution( t, grid, f[grid], Ib1,Ib2,Ib3 );

                                }
                            }
                        }
                        else if( bcLocal(side,axis)==CgWave::neumann )
                        {
            ...
            if( 1==0 )
            {
                f.display("f: RHS before direct Helmholtz solve","%8.1e ");  
        // for( int grid=0; grid<cg.numberOfComponentGrids(); grid++ )
        // {
        //   f[grid].updateGhostBoundaries();
        // }        
            }
      ...
      // ------- SOLVE THE HELMHOLTZ EQUATIONS -----
        ...
        // copy f(.,.,.,freq) to fTemp
                for( int grid=0; grid<cg.numberOfComponentGrids(); grid++ )
                {
                    ...
                    bool ok=ParallelUtility::getLocalArrayBounds(f[grid],fLocal,I1,I2,I3);
            ...
            printF("\n*** freq=%d: omega=%.5g, solver-max-res=%8.2e, time for direct Helmholtz solve = %8.2e (s) (iterations=%i) ***\n",
                          freq,omega,solver.getMaximumResidual(),time,helmholtzSolverIterations);
      

        } // end for freq 

    // --- Evaluate equation residuals ---
        RealArray maxRes;
        int useAdjustedOmega=0; 
        residual( u,f,maxRes,useAdjustedOmega );
    ...
    // Real damp  =  1.;    // ** FIX ME ***  
      ...
      // we can use the standard operators
            if( 1==0 )
            { // trouble here in parallel -- Jan 54, 2026
            ...
            else
            {
                ...
                for( int grid=0; grid<cg.numberOfComponentGrids(); grid++ )
                {
                    ...
                    bool ok=ParallelUtility::getLocalArrayBounds(mg.mask(),maskLocal,D1,D2,D3);
                    ok=ParallelUtility::getLocalArrayBounds(mg.mask(),maskLocal,I1,I2,I3);
                    if( ok )
                    {
                        RealArray dCoeff(M0,D1,D2,D3);
    ...
    // --------------- FILL BOUNDARY CONDITIONS ----------------
                    ...
                    // Identity stencil 
                          ...
                          // (A "real" equation has a possible non-zero right-hand-side)
                            ...
                            // -- Equation 2 : imaginary part: 
                                        ...
                                        {
                      // --- COMPLEX NEUMANN CBC - CURVILINEAR --THREE DIMENSIONS -----
              ...
              //---- FILL IN THE EM RADIATION BCS ----
                ...
                //             
                   ...
                   // Real cyyy  = axis==1 ? is2*omega/(2.*dx[1]*dx[1]*dx[1]) : 0.; 
                    ...
                    // (A "real" equation has a possible non-zero right-hand-side)
                      ...
                      //    -is* omega*omegaSign* Dx ur + c( Dxx ui + .5*Dyy ui ) = 0   
                                ...
                                {
                                    printF("solveHelmholtzDirect:: FILL Engquist Majda RMC -- CURVILINEAR \n");
                  // ---------------------------------------------------------
                  //     ----- Engquist Majda BC : curvilinear -----
                  // 
                  //   i omega D_n u = c*Dnn u + (.5*c)( L - Dnn ) u 
                  //    where 
                  //         u = ur + i ui 
                  // Real part: 
                  //     -omega* Dn ui = c*Dnn ur + .5*c*( L - Dnn) ur 
                  // Imag part:
                  //    +omega* Dn ur = c*Dnn ui + .5*c*( L - Dnn ) ui   
                  // ---------------------------------------------------------
                                    const int is = isv[axis];
                                    RealArray nCoeffr(Rw,Rw,Rw), nnCoeffr(Rw,Rw,Rw);
                                    nCoeffr=0.; nnCoeffr=0.; 
                                    mg.update(MappedGrid::THEinverseVertexDerivative );
                  // Real dr[3]={1.,1.,1.};
                  // // unit square grid spacings: 
                  // for( int dir=0; dir<3; dir++ )
                  ...
                  // macro to make the rxLocal array look 5-dimensional 
                                    ...
                                    mg.update(MappedGrid::THEvertexBoundaryNormal);
                                    OV_GET_VERTEX_BOUNDARY_NORMAL(mg,side,axis,normal); 
                  // rCoeff holds D_r, D_s or D_t 
                                    realSerialArray rCoeff(M0,Ib1,Ib2,Ib3), rrCoeff(M0,Ib1,Ib2,Ib3);
                                    if( axis==0 )
                                    {
                                        mgop.assignCoefficients(MappedGridOperators::r1Derivative  ,rCoeff,  Ib1,Ib2,Ib3,0,0);
                                        mgop.assignCoefficients(MappedGridOperators::r1r1Derivative,rrCoeff, Ib1,Ib2,Ib3,0,0);
                                    }
                                    else if( axis==1 )
                                    {
                                        mgop.assignCoefficients(MappedGridOperators::r2Derivative  ,rCoeff,  Ib1,Ib2,Ib3,0,0);
                                        mgop.assignCoefficients(MappedGridOperators::r2r2Derivative,rrCoeff, Ib1,Ib2,Ib3,0,0);      
                                    }
                                    else
                                    {
                                        mgop.assignCoefficients(MappedGridOperators::r3Derivative  ,rCoeff,  Ib1,Ib2,Ib3,0,0);
                                        mgop.assignCoefficients(MappedGridOperators::r3r3Derivative,rrCoeff, Ib1,Ib2,Ib3,0,0);      
                                    }
                  ...
                  // realSerialArray xxCoeff(M0,Ib1,Ib2,Ib3), yyCoeff(M0,Ib1,Ib2,Ib3), zzCoeff; 
                  ...
                  // if( numberOfDimensions==3 )
                  // {
                  //   zzCoeff.redim(M0,Ib1,Ib2,Ib3);
                  //   mgop.assignCoefficients(MappedGridOperators::zzDerivative ,zzCoeff, Ib1,Ib2,Ib3,0,0);
                  // }
                                    FOR_3D(i1,i2,i3,Ib1,Ib2,Ib3) // loop over points on the boundary
                                    {              
                                        ForStencil(m1,m2,m3)
                        ...
                        // lapCoeff(m1,m2,m3) = xxCoeff(m0,i1,i2,i3) + yyCoeff(m0,i1,i2,i3);
                                            }
                                            else
                                            {
                                                normRx = sqrt( SQR(DD(i1,i2,i3,axis,0)) + SQR(DD(i1,i2,i3,axis,1)) + SQR(DD(i1,i2,i3,axis,2)) );
                        // lapCoeff(m1,m2,m3) = xxCoeff(m0,i1,i2,i3) + yyCoeff(m0,i1,i2,i3) + zzCoeff(m0,i1,i2,i3);
                                            }
                                            nCoeffr(m1,m2,m3) = -is*normRx*rCoeff(m0,i1,i2,i3);  // approximation to D_n 
                                            nnCoeffr(m1,m2,m3) = SQR(normRx)*rrCoeff(m0,i1,i2,i3); // D_n^2 
                                        }
                                        const int ghost=1; 
                                        int i1m=i1-is1*ghost, i2m=i2-is2*ghost, i3m=i3-is3*ghost; //  ghost point is (i1m,i2m,i3m)
                    // Specify that this a "real" equation on the first ghost line: 
                    // (A "real" equation has a possible non-zero right-hand-side)
                      ...
                      //    - omega* Dn ui = c* Dnn ur + .5*c*(L -Dnn) ur 
                                            int mr  = M123CE(m1,m2,m3,ic1,eq1);                        // equation 1, component 1
                                            coeffLocal(mr,i1m,i2m,i3m) = ca*nnCoeffr(m1,m2,m3) + .5*ca*( lapCoeff(m0,i1,i2,i3) - nnCoeffr(m1,m2,m3) );
                                            int j1=i1+m1, j2=i2+m2, j3=i3+m3;                          // the stencil is centred on the boundary pt (i1,i2,i3)
                                            setEquationNumber(mr, eq1,i1m,i2m,i3m,  ic1,j1,j2,j3 );    
                                            int mi  = M123CE(m1,m2,m3,ic2,eq1);                        // equation 1, component 2
                                            coeffLocal(mi,i1m,i2m,i3m) = omega*nCoeffr(m1,m2,m3);      // *check sign**   
                                            setEquationNumber(mi, eq1,i1m,i2m,i3m,  ic2,j1,j2,j3 );    
                      // -- Equation 2 : imaginary part: 
                      //    +omega* Dn ur = c*Dnn ui + .5*c*( L - Dnn ) ui = 0   
                                            mi  = M123CE(m1,m2,m3,ic2,eq2);                            // equation 2, component 2
                                            coeffLocal(mi,i1m,i2m,i3m) =  ca*nnCoeffr(m1,m2,m3) + .5*ca*( lapCoeff(m0,i1,i2,i3) - nnCoeffr(m1,m2,m3) );
                                            setEquationNumber(mi, eq2,i1m,i2m,i3m,  ic2,j1,j2,j3 );    
                                            mr  = M123CE(m1,m2,m3,ic1,eq2);                            // equation 2, component 1
                                            coeffLocal(mr,i1m,i2m,i3m) = -omega*nCoeffr(m1,m2,m3);      // *check sign**   
                                            setEquationNumber(mr, eq2,i1m,i2m,i3m,  ic1,j1,j2,j3 );  
                                        }
                                    } // end FOR_3D
                                }
                  ...
                  // ------------------------
                      ...
                      // (A "real" equation has a possible non-zero right-hand-side)
                        ...
                        //    -is* omega*omegaSign* Dx ur + c( Dxx ui + .5*Dyy ui ) = 0   
                                    ...
                                    {
                    // **** CURVILINEAR 
                                        printF("solveHelmholtzDirect:: ADD EM CBC curvilinear"); 
                    // ---------------------------------------------------------
                    //     ----- Engquist Majda BC : curvilinear -----
                    // 
                    //   i omega D_n u = c*Dnn u + (.5*c)( L - Dnn ) u 
                    ...
                    // Identity stencil 
                      ...
                      //   rx(m,n) = DD(i1,i2,i3,m,n)
                      // cc(0,0) = rx(0,0)*rx(0,0) + rx(0,1)*rx(0,1); 
                      // cc(0,1) = rx(0,0)*rx(1,0) + rx(0,1)*rx(1,1);
                      // cc(1,1) = rx(1,0)*rx(1,0) + rx(1,1)*rx(1,1);
                      // // we need  rxx + ryy
                      // // Dn = || grad(r) || 
                      // if( axis==0 )
                      // {
                      //   DnDrr(m1,m2,m3) = gradrNorm*rrrCoeff(m1)*iCoeff(m2);
                      // }
                      // Specify that this a "real" equation on the second ghost line: 
                      // (A "real" equation has a possible non-zero right-hand-side)
                                            setClassify(eq1,i1m,i2m,i3m, SparseRepForMGF::ghost2);              
      ...
      // Here are the boundary conditions we can impose with the high-level operators: 
        ...
        if( 1==1 ) // April 25, 2025
        {
            BoundaryConditionParameters extrapParams;
            extrapParams.orderOfExtrapolation=orderOfAccuracy+1;     

            coeff.finishBoundaryConditions(extrapParams); 
            printF("solveHelmholtzDirect: FILL IMP CORNER BCS -- EXTRAP TO ORDER %d\n",extrapParams.orderOfExtrapolation);
        }
        else
        {
    ...
    // Index Ig1,Ig2,Ig3; 
      ...
      //  Boundary conditions will be filled in by getHelmholtzForcing, but these are not all correct for the complex case
          ...
          // OV_GET_SERIAL_ARRAY(Real,f[grid],fLocal);
                    ...
                    if( ok && solveForScatteredField && knownSolutionOption==planeWave )
                    {
                        OV_GET_SERIAL_ARRAY(Real,mg.vertex(),xLocal);
          ...
          // bool ok=ParallelUtility::getLocalArrayBounds(f[grid],fLocal,Ib1,Ib2,Ib3);
                                  ...
                                  mg.boundaryCondition(side,axis)==CgWave::abcEM2 )
                {
                    ...
                    {
                        getGhostIndex(mg.gridIndexRange(),side,axis,Ig1,Ig2,Ig3,ghost);
                        bool ok=ParallelUtility::getLocalArrayBounds(f[grid],fLocal,Ig1,Ig2,Ig3);
                        if( ok )
      ...
      //   ::display(fcLocal,"solveHelmholtzDirect: fcLocal","%9.2e ");
        ...
        if( 1==0 )
            fc.display("fc: RHS before direct Helmholtz solve","%8.1e ");
        ...
        printF("\n*** omega=%.5g, solver-max-res=%8.2e, time for direct Helmholtz solve = %8.2e (s) (iterations=%i) ***\n",
                          omega,solver.getMaximumResidual(),time,helmholtzSolverIterations);

   // printF("=== Direct Helmholtz Solver: %s iterations=%d =====\n",(const char*)solver.parameters.getSolverName(),solver.getNumberOfIterations());
