...
extern "C"
{
    ...
    void advWave(const int&nd,
            ...
            const int&bc, const real & frequencyArray, const DataBase *pdb, const int&ipar, const real&rpar,  int&ierr );
...
// UPC scheme: 
// Predict: 
//   (P)  u^{(0)} = 2 u^n - u^{n-1} + dt^2{ c^2 Delta_h( u^n) + ... }      (normal update) 
//        applyBC( u^{(0)}, t+dt )
// Correct
//     for k=1,...
//        u^{(k)} <-  u^{(k-1)} + dt^2 * Q ( u^{(k-1)} - u^{n-1} )/(2*dt)  ( add upwinding )
//        applyBC( u^{(k)}, t+dt )  
//     end
//     u^{n+1} = u^{(k)} 
//
...
advance( int it )
{
  ...
  // *new* way Sept 28, 2025
    adjustTimeStepAndFrequencies( it, nextTimeToPlot );
  ...
  // -------------------------------------------------------------
      ...
      // ---- WaveHoltz: initial condition is provided in v: ----
        ...
        // Appy BC's to v -- important when we use Slepc ??
                bool applyExplicitBoundaryConditions=true;
                applyBoundaryConditions( v,v, t, applyExplicitBoundaryConditions );  // **TRY THIS** -- not sure it is needed??
            }
      // u1=v;
