...
advance( int it )
{
  ...
  // NOTE: dtMax is the initial time-step determined by getTimeStep
    ...
    if( solveHelmholtz && adjustOmega && timeSteppingMethod == implicitTimeStepping )
    ...
    // Note: numPlotsSteps is per period
        ...
        {
            numPlotSteps = max(5,numPlotSteps);
        }
        else
        {
      // we need at least 5 steps for the smallest period
            ...
            int minSteps = ceil( 5*Tmax/Tmin ); // is this correct
      ...
      // ---- adjust the scheme or omega to remove time-discretization errors -----
       ...
       // }
        ...
        if( solveHelmholtz )
        {
            ...
            printF(" dt=%20.12e, omega=%20.12e,  tFinal=%20.12e\n",dt,omega,tFinal);
