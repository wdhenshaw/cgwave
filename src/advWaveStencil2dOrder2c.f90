! This file automatically generated from advWaveStencil.bf90 with bpp.
  subroutine advWaveStencil2dOrder2c( nd,n1a,n1b,n2a,n2b,n3a,n3b,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,mask,xy,rsxy,um,u,un,f,sc,v,vh,lapCoeff,etax,etay,etaz,bc,frequencyArray,pdb,ipar,rpar,ierr )
 !======================================================================
 ...
 !======================================================================
  ...
  ! integer ipar(0:*)
  ! real rpar(0:*)
     real sc(1:9,nd1a:nd1b,nd2a:nd2b)
     real scr(1:9) 
  !     ---- local variables -----
...
! File generated by cgWave/maple/writeStencilFiles.mpl
t1 = lapCoeff(i1,i2,i3,2);
t3 = t1 * dt2 / 0.4e1;
t4 = lapCoeff(i1,i2,i3,4);
t5 = t4 / 0.2e1;
t6 = lapCoeff(i1,i2,i3,1);
t9 = lapCoeff(i1,i2,i3,3);
t10 = t9 / 0.2e1;
t11 = lapCoeff(i1,i2,i3,0);
sc(1,i1,i2) = t3;
   ...
   if( gridIsImplicit.eq.0 )then 
     ! ------- EXPLICIT update the solution ---------
       if( orderInTime.eq.2 .and. orderOfAccuracy.gt.2 )then
         ...
         !   updateWaveOpt(2,2,2,curvilinear,NOFORCING)
         ! else 
         !   updateWaveOpt(2,2,2,curvilinear,FORCING)
         ! end if
       else
         if( addForcing.eq.0 )then
             if( ( .true. .or. debug.gt.3) .and. t.lt.2*dt )then
               write(*,'("advWaveStencil: ADVANCE dim=2 order=2 orderInTime=2, grid=curvilinear... t=",e10.2)') t
             end if
...
un(i1,i2,i3,m)=  - um(i1,i2,i3,m)+ sc(  1,i1,i2)*u(i1-1,i2-1,i3,m) + sc(  2,i1,i2)*u(i1+0,i2-1,i3,m) + sc(  3,i1,i2)*u(i1+1,i2-1,i3,m)+ sc(  4,i1,i2)*u(i1-1,i2+0,i3,m) + sc(  5,i1,i2)*u(i1+0,i2+0,i3,m) + sc(  6,i1,i2)*u(i1+1,i2+0,i3,m)+ sc(  7,i1,i2)*u(i1-1,i2+1,i3,m) + sc(  8,i1,i2)*u(i1+0,i2+1,i3,m) + sc(  9,i1,i2)*u(i1+1,i2+1,i3,m)
               end do
               end do
               end do
             ! endLoopsMask()
...
! Stencil: nd=2, orderOfAccuracy=2, gridType=Curvilinear
un(i1,i2,i3,m)=  - um(i1,i2,i3,m)+ sc(  1,i1,i2)*u(i1-1,i2-1,i3,m) + sc(  2,i1,i2)*u(i1+0,i2-1,i3,m) + sc(  3,i1,i2)*u(i1+1,i2-1,i3,m)+ sc(  4,i1,i2)*u(i1-1,i2+0,i3,m) + sc(  5,i1,i2)*u(i1+0,i2+0,i3,m) + sc(  6,i1,i2)*u(i1+1,i2+0,i3,m)+ sc(  7,i1,i2)*u(i1-1,i2+1,i3,m) + sc(  8,i1,i2)*u(i1+0,i2+1,i3,m) + sc(  9,i1,i2)*u(i1+1,i2+1,i3,m)+dtSq*fv(m)
               end do
               end do
               end do
             ! endLoopsMask()
