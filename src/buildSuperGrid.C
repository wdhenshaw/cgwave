...
// ============================================================================================================
///  \brief  Adjust a grid function by setting the values to "superGridLayerValue" in the supeGrid grid layer
/// 
// ============================================================================================================
int CgWave::
adjustSolutionForSuperGrid( realCompositeGridFunction & q, Real superGridLayerValue /* = 0. */  )
{
  ...
  // --- adjust plots to remove solution in the superGrid layers ---
  // printF("!!!! adjustSolutionForSuperGrid : Adjust plot for SuperGrid\n");

  const IntegerArray & superGrid         = dbase.get<IntegerArray>("superGrid");
  const real & superGridWidth            = dbase.get<real>("superGridWidth"); 
  const IntegerArray & useAbsorbingLayer = dbase.get<IntegerArray>("useAbsorbingLayer"); 
  ...
  for( int grid=0; grid<cg.numberOfComponentGrids(); grid++ )
  {
    ...
    // printF("!!!! Adjust plot for SuperGrid on grid=%d\n",grid);
    ...
    {
      if( useAbsorbingLayer(axis,grid) && mg.boundaryCondition(side,axis)==CgWave::absorbing )
      {
        
        // SG offset is always base of dr I think
        int offset = int( superGridWidth/dr[axis] + .5 );

        // if( isRectangular )
        //   offset = int( superGridWidth/dr[axis] + .5 ); 
        // else
        //   offset = int( superGridWidth/dr[axis] + .5 ); 

        int extraToHide= 0; // int(offset/5); // what should this be ? 

        // printF("!!!! Adjust plot for SuperGrid on grid=%d, (side,axis)=(%d,%d) base-offset=%d, extraToHide=%d\n",grid,side,axis,offset,extraToHide);
        ...
        Ibv[axis]= side==0 ? Range(dim(side,axis)       ,gid(side,axis)+offset) : 
                             Range(gid(side,axis)-offset,dim(side,axis)       );

        // --- Set values in the SuperGrid layer to nan so they are not plotted---
        // qLocal(Ib1,Ib2,Ib3,all)= NAN; // std::numeric_limits<double>::quiet_NaN();
        qLocal(Ib1,Ib2,Ib3,all)= superGridLayerValue; 
...
buildSuperGrid( )
{
  ...
  int & initializeSuperGrid = dbase.get<int>("initializeSuperGrid");

  if( initializeSuperGrid==0 )
  {
    OV_ABORT("buildSuperGrid:: ERROR : superGrid is already initialized!!\n");
  }

  initializeSuperGrid=0;
...
//  sigmaz = @(z)  ( (1-epsL)* q*( 1 - (1 -z/l).^p ).^(q-1) ).* ( (p/l)*(1 -z/l).^(p-1) );  % d(sigma)/dz    
  ...
  // superGrid(grid) = 1 if this grid uses superGrid
  ...
  for( int grid=0; grid<numberOfComponentGrids; grid++ )
  {
    ...
    superGrid(grid)=1; // this grid is superGrid active
    ...
    if( !isRectangular )
    {
      ...
      {
        RealArray *& rxOriginal = dbase.put<RealArray*>("rxOriginal");
        rxOriginal = new RealArray [cg.numberOfComponentGrids()];   
      }
      RealArray *& rxOriginal = dbase.get<RealArray*>("rxOriginal");
      rxOriginal[grid].redim(rxLocal);
      rxOriginal[grid] = rxLocal;   // Keep a copy 
