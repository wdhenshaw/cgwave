          ...
          ! FD26 : second-order in time and sixth-order in space
                  ...
                  ! ------------ MAIN LOOP, 6 SIXTH 6 ----------------
                      ...
                      !   uxx = D+D- ( 1 - (1/12)*dx^2(D+D-) + (1/90)*dx^4 (D+D-)^2 -(1/560)*(D+D-)^3 + ... )
                                            uxx = d200(i1,i2,i3,0) - (dx(0)**2/12.)*d400(i1,i2,i3,0) + (dx(0)**4/90.)*d600i 
                                            uyy = d020(i1,i2,i3,0) - (dx(1)**2/12.)*d040(i1,i2,i3,0) + (dx(1)**4/90.)*d060i 
                      ! ----- fourth derivs ------
                      !  1/6, 7/240, 41/7560
                      !   uxxxx = (D+xD-x)^2*( 1 - (1/6)*D+xD-x + (7/240)*(D+xD-x)^2 + ... )
                                            uxxxx = d400(i1,i2,i3,0) - (dx(0)**2/6.)*d600i ! + (dx(0)**4 * (7./240))*d800i
                                            uyyyy = d040(i1,i2,i3,0) - (dx(1)**2/6.)*d060i ! + (dx(1)**4 * (7./240))*d080i
                                            uxxyy = d220(i1,i2,i3,0) - (dx(0)**2/12.)*d420i - (dx(1)**2/12.)*d240i 
                      ! ---- sixth derivs----
                      !  1/4 , 13/240 
                                            uxxxxxx = d600i 
                                            uyyyyyy = d060i 
