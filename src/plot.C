...
// ========================================================================================
{
  ...
  // real cpu0=getCPU();
    ...
    const int useSosupDissipation = ad4>0.;

    aString suffix="";
  // if( timeSteppingMethod==implicitTimeStepping )
  //   suffix = "i"; // i = implicit TS
    if( useSosupDissipation )
        suffix = suffix + "s";  // name = FD22s, FD44s etc. for sosup dissipation

    char buff[100];
    psp.set(GI_TOP_LABEL,sPrintF(buff,"CgWave FD%i%i%s t=%6.2e ",
